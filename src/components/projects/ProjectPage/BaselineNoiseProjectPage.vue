...
const workflowSteps = [
  ...
  ['05', 'Model and Recommend', 'Fit dwell-time-aware distributions and recommend cautious correction support.'],
];
...
const exploratoryFigures = [
  {
    ...
    body: 'The log scale makes the dwell-time shift visible: centers move left and the distributions narrow as dwell time increases.',
    alt: 'Positive BCA intensity log10 histograms by dwell time',
  },
  {
    image: qqFigure,
    title: 'Q-Q plots against a normal reference',
    body: 'Systematic tail departures show that a simple log-normal assumption is useful as a reference but too rigid as the final model.',
...
const modelFigures = [
  ...
  },
  {
    ...
    body: 'CDF diagnostics check the whole distribution, not only the peak; remaining tail mismatch is why validation is required before production correction.',
...
<template>
  <main class="relative overflow-hidden bg-cream text-ink">
    ...
    <section class="relative z-[1] mx-auto grid min-h-[680px] w-[min(1120px,calc(100%_-_48px))] grid-cols-1 items-center gap-10 py-14 pt-[76px] text-center lg:grid-cols-[minmax(0,1.04fr)_minmax(360px,0.76fr)] lg:gap-14 lg:text-left">
      <div>
        ...
        <p class="mt-7 max-w-[620px] text-[18px] font-light leading-[1.72] text-ink3">
          Statistics capstone project for Thermochron Systems LLC modeling Prisma Pro mass spectrometer baseline noise so future ion-current measurements can use dwell-time-specific correction rather than a single pooled background value.
        </p>
        <div class="mt-9 flex flex-wrap justify-center gap-3 lg:justify-start">
          <a class="inline-flex min-h-[46px] items-center justify-center rounded-[10px] bg-ink px-[22px] text-sm font-semibold text-white no-underline transition-colors hover:bg-accent" href="#results">View results</a>
          <a class="inline-flex min-h-[46px] items-center justify-center rounded-[10px] border border-border2 px-[22px] text-sm font-semibold text-ink no-underline transition-colors hover:border-accent2 hover:text-accent2" href="#workflow">See workflow</a>
        </div>
      </div>
      ...
      <div class="mx-auto w-[min(1120px,calc(100%_-_48px))]">
        <header class="mb-[34px] grid grid-cols-1 items-end gap-6 md:grid-cols-[0.85fr_1fr] md:gap-14">
          <div>
            ...
            <h2 class="font-display text-[34px] font-bold leading-[1.06] text-ink md:text-[48px]">A report page built around scientific measurement risk.</h2>
          </div>
          <p class="m-0 text-[15px] font-light leading-[1.76] text-ink3">The project moves from semi-structured mass spectrometer files to cleaned BCA observations, distribution checks, dwell-time diagnostics, and a model that supports correction without physically impossible negative signals.</p>
        </header>
        ...
        <div class="mb-7 grid grid-cols-1 gap-4 md:grid-cols-3">
          ...
          <article class="min-h-[168px] rounded-[12px] border border-border bg-white p-[22px]">
            ...
            <strong class="block text-[15px] text-ink">Correction should match the measurement channel</strong>
    ...
    <section class="mx-auto w-[min(1120px,calc(100%_-_48px))] py-[82px]">
      <header class="mb-[34px] grid grid-cols-1 items-end gap-6 md:grid-cols-[0.85fr_1fr] md:gap-14">
        <div>
          ...
          <h2 class="font-display text-[34px] font-bold leading-[1.06] text-ink md:text-[48px]">Mass spectrometry turns tiny currents into scientific evidence.</h2>
        </div>
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <article class="rounded-[12px] border border-border bg-white p-6">
          ...
          <p class="m-0 text-[13px] leading-[1.6] text-ink3">A Baseline Characterization Analysis targets a mass where no real gas species should exist, so observed current approximates the noise process.</p>
        </article>
      </div>
      ...
      <div class="mt-6 grid grid-cols-1 gap-6 md:grid-cols-[1.06fr_0.94fr]">
        <article class="rounded-[12px] border border-border bg-white p-6">
          ...
          <p class="text-[13px] leading-[1.6] text-ink3">All six dwell-time channels measure baseline mass settings, making BCA files useful for asking whether the noise distribution changes systematically with dwell time.</p>
    ...
    <section class="mx-auto w-[min(1120px,calc(100%_-_48px))] py-[82px]">
      <header class="mb-[34px] grid grid-cols-1 items-end gap-6 md:grid-cols-[0.85fr_1fr] md:gap-14">
        <div><div class="mb-3.5 flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-[2px] text-accent before:h-0.5 before:w-[26px] before:bg-accent before:content-['']">Exploratory Findings</div><h2 class="font-display text-[34px] font-bold leading-[1.06] text-ink md:text-[48px]">The cleaned BCA data show strong dwell-time dependence.</h2></div>
    ...
    <section id="results" class="bg-cream2 py-[82px]">
      <div class="mx-auto w-[min(1120px,calc(100%_-_48px))]">
        <header class="mb-[34px] grid grid-cols-1 items-end gap-6 md:grid-cols-[0.85fr_1fr] md:gap-14"><div><div class="mb-3.5 flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-[2px] text-accent before:h-0.5 before:w-[26px] before:bg-accent before:content-['']">Modeling Output</div><h2 class="font-display text-[34px] font-bold leading-[1.06] text-ink md:text-[48px]">The fitted model supports dwell-time-specific correction.</h2></div><p class="m-0 text-[15px] font-light leading-[1.76] text-ink3">Maximum likelihood fitting used 491,997 positive BCA observations. KDE remains a diagnostic benchmark; the structured log-skew-t model is the recommendation.</p></header>

        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <article class="rounded-[12px] border border-green-800/30 bg-[#f4fbf7] p-5"><span class="inline-block rounded-full bg-[#e9f8ef] px-2.5 py-1 text-[10px] font-bold uppercase tracking-[1px] text-[#147a4d]">Recommended framework</span><h3 class="mt-4 text-[15px] font-bold text-ink">Reparameterized log-skew-t</h3><p class="text-[13px] leading-[1.6] text-ink3">Location, scale, and tail-heaviness vary smoothly with dwell time, while shared skewness pools information across the six dwell groups.</p><div class="mt-[18px] grid grid-cols-2 gap-2.5"><div class="rounded-lg bg-cream p-3"><strong class="block font-mono text-[17px] text-ink">1,351,186</strong><span class="text-[11px] text-ink3">AIC from fitted model</span></div><div class="rounded-lg bg-cream p-3"><strong class="block font-mono text-[17px] text-ink">1,351,264</strong><span class="text-[11px] text-ink3">BIC from fitted model</span></div></div></article>
          <article class="rounded-[12px] border border-border bg-white p-5"><span class="inline-block rounded-full bg-accent-pale px-2.5 py-1 text-[10px] font-bold uppercase tracking-[1px] text-accent">Diagnostic benchmark</span><h3 class="mt-4 text-[15px] font-bold text-ink">KDE and cross-dwell checks</h3><p class="text-[13px] leading-[1.6] text-ink3">KDE is a nonparametric diagnostic benchmark, not the final recommendation. Density overlays, CDF plots, and Spearman correlations check shape, tails, and cycle-level dependence.</p><div class="mt-[18px] grid grid-cols-2 gap-2.5"><div class="rounded-lg bg-cream p-3"><strong class="block font-mono text-[17px] text-ink">0.215</strong><span class="text-[11px] text-ink3">max density gap</span></div><div class="rounded-lg bg-cream p-3"><strong class="block font-mono text-[17px] text-ink">0.007</strong><span class="text-[11px] text-ink3">mean Spearman correlation</span></div></div></article>
        </div>
    ...
    <section class="mx-auto w-[min(1120px,calc(100%_-_48px))] py-[82px]">
      <header class="mb-[34px] grid grid-cols-1 items-end gap-6 md:grid-cols-[0.85fr_1fr] md:gap-14"><div><div class="mb-3.5 flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-[2px] text-accent before:h-0.5 before:w-[26px] before:bg-accent before:content-['']">Deliverables</div><h2 class="font-display text-[34px] font-bold leading-[1.06] text-ink md:text-[48px]">Give visitors multiple levels of depth.</h2></div><p class="m-0 text-[15px] font-light leading-[1.76] text-ink3">The artifact set supports a quick portfolio read, notebook inspection, and full statistical-report review.</p></header>
      <div class="grid grid-cols-1 gap-4 md:grid-cols-3"><article class="rounded-[12px] border border-border bg-white p-[22px]"><div class="mb-[22px] font-mono text-[11px] uppercase text-ink4">Notebook pipeline</div><strong class="block text-[15px] text-ink">01-preprocessing, 02-assumptions, 03-modeling</strong><p class="mt-2 text-[13px] leading-[1.55] text-ink3">Three notebooks cover source parsing, assumption checks, EDA, and log-skew-t fitting.</p></article><article class="rounded-[12px] border border-border bg-white p-[22px]"><div class="mb-[22px] font-mono text-[11px] uppercase text-ink4">Statistical report</div><strong class="block text-[15px] text-ink">Baseline Noise Distribution Report.pdf</strong><p class="mt-2 text-[13px] leading-[1.55] text-ink3">A 38-page capstone report with summary, methods, diagnostics, conclusions, and correction recommendations.</p></article><article class="rounded-[12px] border border-border bg-white p-[22px]"><div class="mb-[22px] font-mono text-[11px] uppercase text-ink4">Source archive + deck</div><strong class="block text-[15px] text-ink">data.zip, presentation.pptx, project-background.docx</strong><p class="mt-2 text-[13px] leading-[1.55] text-ink3">Source and presentation materials connect the analysis to its scientific measurement context.</p></article></div>
    </section>

    <section class="bg-cream2 py-[82px]"><div class="mx-auto w-[min(1120px,calc(100%_-_48px))]"><header class="mb-[34px] grid grid-cols-1 items-end gap-6 md:grid-cols-[0.85fr_1fr] md:gap-14"><div><div class="mb-3.5 flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-[2px] text-accent before:h-0.5 before:w-[26px] before:bg-accent before:content-['']">Correction Implication</div><h2 class="font-display text-[34px] font-bold leading-[1.06] text-ink md:text-[48px]">The practical output is a dwell-time-matched correction strategy.</h2></div><p class="m-0 text-[15px] font-light leading-[1.76] text-ink3">The report estimates the baseline-noise distribution that matches each measurement channel's dwell time before applying a correction.</p></header><div class="grid grid-cols-1 gap-4 md:grid-cols-3"><article class="rounded-[12px] border border-border bg-white p-[22px]"><div class="mb-[22px] font-mono text-[11px] uppercase text-ink4">First correction summary</div><strong class="block text-[15px] text-ink">Use median baseline noise by dwell time first.</strong><p class="mt-2 text-[13px] leading-[1.55] text-ink3">The median is less sensitive to heavy upper tails than the mean, making it a stable first-choice correction summary.</p></article><article class="rounded-[12px] border border-border bg-white p-[22px]"><div class="mb-[22px] font-mono text-[11px] uppercase text-ink4">Model-based correction</div><strong class="block text-[15px] text-ink">Correction should come from the BCA model.</strong><p class="mt-2 text-[13px] leading-[1.55] text-ink3">Use the dwell-time-specific BCA distribution rather than one live baseline value for every channel.</p></article><article class="rounded-[12px] border border-border bg-white p-[22px]"><div class="mb-[22px] font-mono text-[11px] uppercase text-ink4">Practical meaning</div><strong class="block text-[15px] text-ink">Reduce overcorrection risk.</strong><p class="mt-2 text-[13px] leading-[1.55] text-ink3">Matching correction to dwell time should reduce negative signals and clarify downstream uncertainty.</p></article></div></div></section>

    <section class="mx-auto w-[min(1120px,calc(100%_-_48px))] py-[82px]"><div class="relative overflow-hidden rounded-[14px] bg-ink p-8 text-white md:p-11"><div class="pointer-events-none absolute right-[-170px] top-[-170px] h-[500px] w-[500px] rounded-full border border-white/[0.05]" aria-hidden="true"></div><div class="relative z-[1] mb-3.5 flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-[2px] text-white/65 before:h-0.5 before:w-[26px] before:bg-white/65 before:content-['']">Final Conclusions</div><h2 class="relative z-[1] max-w-[760px] font-display text-[34px] font-bold leading-[1.06] text-white md:text-[48px]">Baseline noise should be modeled by dwell time before correction.</h2><h3 class="relative z-[1] mt-[26px] text-[15px] font-bold text-white">Final Project Conclusion</h3><p class="relative z-[1] max-w-[740px] text-base leading-[1.72] text-white/70">The cleaned Prisma Pro BCA data show that baseline noise is positive-valued, right-skewed, heavy-tailed, and strongly dependent on dwell time. A single pooled baseline value or one live baseline channel is not enough to support every experimental signal channel.</p><h3 class="relative z-[1] mt-[26px] text-[15px] font-bold text-white">Recommendations</h3><p class="relative z-[1] max-w-[740px] text-base leading-[1.72] text-white/70">Use median baseline noise by dwell time as the first practical correction summary. Validate model-based correction on unknown experimental samples and collect BCA measurements at additional dwell settings before production use.</p></div></section>
  </main>
</template>
